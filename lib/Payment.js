...
class Payment {
   ...
   * @return {Promise}
   */
  generateAPPChooseWXPayInfo(prepayId, signType) {
    const params = {
      appid: this.wechatConfig.appId,
      partnerid: this.wechatConfig.merchantId,
      timestamp: utils.timestamp(),
      noncestr: utils.nonceStr(),
      prepayid: prepayId,
      package: 'Sign=WXPay',
    };
    const ret = this.generateGeneralPaymentSignature(params, SIGN_TYPE.MD5);
    return Promise.resolve({
      timestamp: params.timestamp,
      nonceStr: params.noncestr,
      package: params.package,
    ...
    return utils
      ...
      .then(xmlData => {
        return utils.sendWechatPaymentRequest(this.paymentUrls.REFUND, {
          body: xmlData,
          httpsAgent: this.getPaymentAgent(),
        });
      })
      .then(rawData => {
